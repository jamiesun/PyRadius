...
def node_seq():
    return [(node.id,node.name) for node in store.get_node_all() ]
...
node_add_form = form.Form(
    ...
    form.Textbox("opr_ip", is_ip,description="管理员工作IP:（如192.168.1.1）",**input_style),
    ...
    validators = [
        form.Validator("password not match", lambda i: i.opr_passwd == i.opr_passwd2)]
)
...
nas_add_form = form.Form(
    ...
    form.Textbox("auth_secret",is_alphanum2(6,31), description="认证密钥 ：（由6-31位内，建议字母开头，字母和数字组合）",**input_style),
    form.Textbox("acct_secret", is_alphanum2(6,31),description="计费密钥 ：（由6-31位内，建议字母开头，字母和数字组合）",**input_style),
    form.Textbox("vendor_id", is_number,description="厂家标识：",**input_style),
    form.Dropdown("time_type",  [("0","标准时区"),("1","时间同区")],description="时区/时间类型："),
...
product_add_form = form.Form(
    ...
    form.Textbox("pid", is_alphanum2(6,32),description="产品套餐编码：（6-32个字母数字组合以内）",**input_style),
    ...
    form.Textbox("cuncur_number",is_number, description="用户并发数：（0表示不限定|并发数不能超过20）",**input_style),
